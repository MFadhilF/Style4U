...
<div className="flex space-x-4 sm:space-x-6 md:space-x-8 px-4 font-playfair font-semibold w-max py-2">
  {[
    ...
    >
      ...
      {/* INFO PRODUK */}
      <h4 className="text-sm sm:text-base font-semibold mb-0.5 truncate" title={p.title}>{p.title}</h4>
      ...
      <div className="mt-auto w-full flex justify-between items-center text-xs sm:text-sm font-playfair pt-2">
        <span>{p.price}</span>
        <button className="text-gray-700 hover:underline font-semibold">+ Keranjang</button>
      </div>
    </div>
  ))}
</div>
